...
of -- no Gaussian assumption is imposed on the shape.

BACKGROUND JOB PATTERN: same as kmeans_router.py -- up to 50,000 iterations
of pure-Python loops is real CPU time; it now runs via BackgroundTasks
instead of inline, so it can't block other users' requests while it grinds.
POST /simulate validates + deducts credit + returns a job_id; GET
/job/{job_id} polls for the result. See compute_jobs.py's docstring for
exactly what this pattern does and doesn't fix.
"""
...
def _execute_simulation_job(job_id: str, user_id: str, deals: list, iterations: int, bucket_count: int):
    ...
    try:
        ...
        compute_jobs.mark_done(job_id, result)
...
def simulate(background_tasks: BackgroundTasks, payload: dict = Body(...), user_id: str = Depends(get_current_user)):
    """
    Body: {
